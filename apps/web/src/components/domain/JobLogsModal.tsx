...
function JobLogsBody({ job, open }: { job: Job | null; open: boolean }) {
    ...
    <>
      <DialogHeader>
        <DialogTitle className="flex items-center justify-between">
          ...
          <span
            className={isConnected ? "text-green-500 text-xs" : "text-red-500 text-xs"}
          >
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="max-w-3xl">
        <JobLogsBody key={job?.id ?? "no-job"} job={job} open={open} />
      </DialogContent>
    </Dialog>
  );
}
